...
async function fetchUserProfile() {
  ...
  const url = "http://localhost/TASK/api/get_user_info.php";
...
async function updateTask(taskID) {
  ...
  // const token = localStorage.getItem("token");
  const url = `http://localhost/TASK/api/task-update.php`;
...
document.getElementById("task-form").addEventListener("submit", async (e) => {
  ...
  try {
    const response = await fetch("http://localhost/TASK/api/task-add.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function updateTaskStatus(taskID) {
  try {
    const response = await fetch("http://localhost/TASK/api/task-update.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
